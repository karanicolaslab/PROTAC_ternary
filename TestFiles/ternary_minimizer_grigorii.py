...
def get_atoms(ter_file, onlyHet=False, onlyAtm=False):
    ...
    Args:
        ...
        onlyAtm (bool, optional): Atoms from the rest of the ternary model
...
def babel_runner(pdb_file, mol_file):
    ...
    cmd = f"{BABEL} -h -ipdb {pdb_file} -omol2 -O {mol_file}"
    os.system(cmd)
...
def mol2_to_params(ter_file):
    ...
    MOL2PARAMS = "/mnt/shared_applications/Rosetta_JKlab/Rosetta/main/" + "source/scripts/python/public/molfile_to_params_yates.py"
    cmd = f"{MOL2PARAMS} {ter_file} -n TRN --clobber --no-param"
    os.system(cmd)
...
def rosetta_minimizer(params, ter_file):
    ...
    Args:
        params (file): Params file taken from the pose with the lowest rmsd value
        ter_file (str): name of the modified pdb file with updated PROTAC
    """
    MINIMIZER_APP = "/mnt/shared_applications/Rosetta_JKlab/Rosetta/main/" +
    "source/bin/minimize_ppi.default.linuxgccrelease"

    DATABASE = "/mnt/shared_applications/Rosetta_JKlab/Rosetta/main/database/"

    cmd = f"{MINIMIZER_APP} -database {DATABASE} -s {mod_pdb} -extra_res_fa" +
    f" {params} -jump_all"
...
def remove_file(file):
    ...
    Args:
        file (str): name of the TRN_0001.pdb that was generated from mol_2_params function
    """
    cmd = f"rm {file}"
    os.system(cmd)
...
def main():
    """Main function that inputs the list of ternary models and the best rmsd params file and performs minimizations.
    """
    pdb_list = pdb_names("pdb_list.txt")
    params = "TRN.params"

    for pdb in pdb_lst:
        hetatms = get_heatoms(pdb, onlyHet=True)  # extract hetatoms from PDB
