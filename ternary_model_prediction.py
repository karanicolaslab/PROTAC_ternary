...
def usage():
	...
	print "	-linker_atoms_alignment <filename>	# Names of linker atoms which would be used for the alignment"
	print "	-decoy_atoms_alignment <filename>	# Names of decoy atoms which would be used for the alignment"
	print "	-decoy_file <filename>			# Decoys: can take decoy.pdb or a list file with lines: /pathway/decoy.pdb"
	print "	-linker_conformer_file <filename>	# Linker conformers: can take linker.pdb or a list file with lines: /pathway/linker.pdb"
	print "	-cut_off <float in units of angstrom>	# Cut off value of rmsd: default set is 0.4"
	print "	-rmsd_file <filename>			# Output rmsd file: rmsd values of each decoy and linker conformer with rmsd.txt as default name"
	print "	-ternary_model default/specify		# Output ternary model: if not applied, no ternary structure will be generated"
	print "	-linker_atoms_delete <filename>		# Linker atom list: residue ID and atom ID of atoms which will be deleted"
	print "	-warheads_atoms_delete <filename>	# Decoy atom list: residue ID and atom ID of atoms which will be deleted, coming from warheads"
	print "	-version"
	print "	-help"
	print "\n"
	quit()
...
def main():
	...
	linker_atoms_alignment = ""
	decoy_atoms_alignment = ""
	decoy_file = ""
	linker_conformer_file = ""
	cut_off = 0.4
	warheads_atom_delete = ""
	linker_atom_delete = ""
	ternary_model_name = ""
	rmsd_file = ""
	...
	flags = sys.argv
	# note: the first item is the name of this script: decoy_linker_alignment.py
	script_name = flags.pop(0)
	...
	while (len(flags) > 0):
		...
		if (curr_flag.lower() == "-help"):
			...
			usgae()
			quit()
		elif (curr_flag.lower() == "-version"):
			print "\nThis is version 1.0 of the ternary_model_prediction code.\n"
			quit()
		
		
		elif (curr_flag.lower() == "-linker_atoms_alignment"):
			if (len(flags) == 0):
				print "Error: filename for linker_atoms_alignment flag was not specified."
				quit()
			linker_atoms_alignment = flags.pop(0)
			if (linker_atoms_alignment.startswith('-')):
				print "Error: the linker_atoms_alignment file was not specified (or starts with \-, which is strange)"
				quit()
		
		elif (curr_flag.lower() == "-decoy_atoms_alignment"):
			if (len(flags) == 0):
				print "Error: filename for decoy_atoms_alignment flag was not specified."
				quit()
			decoy_atoms_alignment = flags.pop(0)
			if (decoy_atoms_alignment.startswith('-')):
				print "Error: the decoy_atoms_alignment file was not specified (or starts with \-, which is strange)"
				quit()
		
		# note: check if the input decoy file is a *.pdb or a list
		elif (curr_flag.lower() == "-decoy_file"):
			if (len(flags) == 0):
				print "Error: filename for decoy_file flag was not specified."
				quit()
			decoy_file = flags.pop(0)
			if (decoy_file.startswith('-')):
				print "Error: the decoy_file file was not specified (or starts with \-, which is strange)"
				quit()
			if (decoy_file[-4:] == ".pdb"):
		...
		# note: check if the input linker file is a *.pdb or a list
		elif (curr_flag.lower() == "-linker_conformer_file"):
			if (len(flags) == 0):
				print "Error: filename for linker_conformer_file flag was not specified."
				quit()
			linker_conformer_file = flags.pop(0)
			if (linker_conformer_file.startswith('-')):
				print "Error: the linker_conformer_file file was not specified (or starts with \-, which is strange)"
				quit()
			if (linker_conformer_file[-4:] == ".pdb"):
				linker_list.append(linker_conformer_file)
			else:
				linker_list = read_file(linker_conformer_file)

		# note: if not use, not ternary pdb will be printed, if used, default name or specified name will be applied
		elif (curr_flag.lower() == "-ternary_model"):
			if (len(flags) == 0):
				print "Error: ternary model name was not specified."
				quit()
			ternary_model_name = flags.pop(0)
			if (ternary_model_name.startswith('-')):
				print "Error: ternary model name file was not specified (or starts with \-, which is strange)"
				quit()
			if (ternary_model_name == "default"):
				...
				print "Specified name (decoy + linker) will be applied to ternary model.\n"
			 	...
			 	print "\nError: unrecognized setting", ternary_model_name
			 	quit()
		
		elif (curr_flag.lower() == "-cut_off"):
			if (len(flags) == 0):
				print "Error: value of cut_off value was not specified."
				quit()
			cut_off = float(flags.pop(0))
			if (cut_off < 0):
				print "Error: cut_off must be positive."
				quit()

		elif (curr_flag.lower() == "-rmsd_file"):
			if (len(flags) == 0):
				print "Error: rmsd_file flag was not specified.\n"
				quit()
			rmsd_file = flags.pop(0)
			if (rmsd_file.startswith('-')):
				print "Error: rmsd_file file was not specified (or starts with \-, which is strange)"
				quit()
			
		elif (curr_flag.lower() == "-linker_atoms_delete"):
			if (len(flags) == 0):
				print "Error: filename for linker_atom_delete flag was not specified."
				quit()
			linker_atom_delete = flags.pop(0)
			if (linker_atom_delete.startswith('-')):
				print "Error: the linker_atom_delete file was not specified (or starts with \-, which is strange)"
				quit()
			
		elif (curr_flag.lower() == "-warheads_atoms_delete"):
			if (len(flags) == 0):
				print "Error: filename for warheads_atom_delete flag was not specified."
				quit()
			warheads_atom_delete = flags.pop(0)
			if (warheads_atom_delete.startswith('-')):
				print "Error: the warheads_atom_delete file was not specified (or starts with \-, which is strange)"
				quit()
	...
	if (linker_atoms_alignment == ""):
		print "\nError: must specify a file containing the list of linker atoms which would be used for alignment.\n"
		quit()
	if (decoy_atoms_alignment == ""):
		print "\nError: must specify a file containing the list of decoy atoms which would be used for alignment.\n"
		quit()
	if (len(decoy_list) == 0):
		print "\nError: must specify a decoy pdb file.\n"
		quit()
	if (len(linker_list) == 0):
		print "\nError: must specify a linker conformer pdb file.\n"
		quit()
	if (cut_off < 0):
		print "Error: cut_off must be positive."
		quit()
	...
	if (warheads_atom_delete == ""):
		print "\nWarning: all atoms from warheads will be included in the protac.\n"
		warheads_atom_delete_list = []
	else:
		warheads_atom_delete_list = atom_list_generation(warheads_atom_delete)

	if (linker_atom_delete == ""):
		print "\nWarning: all atoms from linker_conformer will be included in the protac.\n"
	...
	for i in range (len(decoy_array)):
		...
		for j in range (len(linker_array)):
			...
			if (rmsd_value > cut_off or ternary_model_name == ""):
				#print "No ternary structure model will be generated.\n"
				pass
			else:
				if (ternary_model_name == "default"):
