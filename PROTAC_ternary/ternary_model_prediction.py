...
def atom_list_generation(atom_file):
	"""Take in atom files and output atom_names and residue_IDs list, for both alignment atom files and delete atom files.""" 
	atom_list = [line.rstrip('\n').split() for line in open(atom_file)]
	return atom_list

def pdb_to_dictionary(pdb_file):
	"""Take in linker/decoy pdb and output a dictionary with (ligand name + atom name) as key and (x, y, z) as val."""
...
def whole_linker_coors_assignment(linker_dic, atom_number):
	"""Take in dictionary and output coordinates array (whole_atom_number X 3)."""
...
def linker_conformer_after_rmsd_optimizaton(initial_linker_dic, new_wholelinker_coors_array, linker_atom_number):
	"""Take in initial linker dictionary and new linker coordinates, output new linker dictionary with new coors."""
	...
	for key in initial_linker_dic:
		...
		if m < linker_atom_number:
			x = str('%8.3f' %
					(float(new_wholelinker_coors_array[m, 0]))).rjust(8)
			y = str('%8.3f' %
					(float(new_wholelinker_coors_array[m, 1]))).rjust(8)
			z = str('%8.3f' %
					(float(new_wholelinker_coors_array[m, 2]))).rjust(8)
...
def protac_dic_combine(warheads_dic, warheads_atom_delete_list, linker_dic, linker_atom_delete_list):
	"""Take in (linker dic and warhead dic), and delete lists; remove overlap atoms based on delete lists, and output protac dic."""
...
def protac_dic_renmuber(old_protac_dic):
	...
	for key in old_protac_dic:
		...
		newline = line[0:6] + atom_number + \
			line[11:14] + "   LGX X" + line[22:]
...
def dic_to_pdb(dic, pdb_name):
	"""Take in dic and output pdb file."""
...
def main():
	...
	for decoy_name in decoy_list:
		...
		docking_decoy_coors = pdb_coors_assignment(
			decoy_atom_alignment_list, decoy_dic)
	...
	for linker_name in linker_list:
		...
		orig_linker_coors = pdb_coors_assignment(
	...
	for i in range(len(decoy_array)):
		...
		for j in range(len(linker_array)):
			...
			res = scipy.optimize.minimize(eval_rmsd_after_transformation, (0., 0., 0., 0., 0., 0.), args=(
				orig_linker_coors, docking_decoy_coors), method='Powell')
			new_linker_coors = apply_transformation(
				(res.x[0], res.x[1], res.x[2], res.x[3], res.x[4], res.x[5]), orig_linker_coors)

			print(decoy_pdb, linker_conformer_pdb, res.x[0], res.x[1], res.x[2], res.x[3], res.x[4], res.x[5])
